...
class ANN
{
	...
	public:
		...
		inline double tanh(double y){ return 1.0-1.0/(exp(2*y)+1.0); }
		inline double dtanh(double y){ return 2.0/(exp(2*y)+2.0+1/exp(2*y)); }
};
...
void ANN::Gradient_Descent()
{
	...
	for(int j = 0; j < Num_Y; j ++)
		for(int i = 0; i < Num_M; i ++)
		{
			double dJ_dwnet = (TrainY[j] - Expect[j]) * dtanh(Ynet[j]);
	...
	for(int j = 0; j < Num_M; j ++)
		for(int i = 0; i < Num_X; i ++)
		{
			double dMid_dmnet = dtanh(Mnet[j]);
